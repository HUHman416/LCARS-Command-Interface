...
def voice_transcribe(data):
    ...
    if not engine or not Path(engine).is_file() or not model.is_file():return {"ok":False,"message":"The local whisper.cpp voice runtime is unavailable; reinstall 30.12 or select custom files in Settings"}
...
def integration_health():
    ...
    return {
        ...
        "voice":{"available":voice_status()["available"],"detail":voice_status()["reason"] or "Bundled offline whisper.cpp and English command model ready","remedy":"Reinstall Version 30.12 voice resources or select custom whisper.cpp files in Settings."},

...
def voice_transcribe(data):
    ...
    if not engine or not Path(engine).is_file() or not model.is_file():return {"ok":False,"message":"The local whisper.cpp voice runtime is unavailable; reinstall Version 30 or select custom files in Settings"}
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="lcars-voice-") as folder:
            ...
            if raw[:4]==b"RIFF" and raw[8:12]==b"WAVE":wav.write_bytes(raw)
                ...
                if not status["ffmpeg"]:return {"ok":False,"message":"This legacy microphone format needs FFmpeg; the direct PCM recorder does not"}
...
def integration_health():
    ...
    return {
        ...
        "voice":{"available":voice_status()["available"],"detail":voice_status()["reason"] or "Bundled offline whisper.cpp and English command model ready","remedy":"Reinstall Version 30 voice resources or select custom whisper.cpp files in Settings."},
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif route=="/api/portal-status": self.send_json(INTENT_BROKER.status())
        elif route=="/api/portal-request":
            request=INTENT_BROKER.request(parse_qs(parsed.query).get("id",[""])[0])
            self.send_json({"ok":bool(request),"request":request},200 if request else 404)

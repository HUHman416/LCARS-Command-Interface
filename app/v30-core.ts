...
const centers: Record<string, { target: string; label: string }> = {
  ...
  commissioning: { target: "commissioning", label: "Commissioning Center" },
  "commissioning center": { target: "commissioning", label: "Commissioning Center" },
  portals: { target: "portals", label: "Portal Center" },
  "portal center": { target: "portals", label: "Portal Center" },
  permissions: { target: "portals", label: "Portal Center" },
};
...
const interpretClause = (rawClause: string, context: ComputerContext) => {
  ...
  if (/^(?:what can i say|command help|computer help|help with commands)\??$/.test(clause)) return { value: step("navigate", "Open command reference", "Open the offline LCARS Computer command reference", "commissioning", "safe", true), confidence: 1 };

  ...
  return <div className="backdrop computer-core-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !running && close()}>
    <section className="computer-core-console" role="dialog" aria-modal="true" aria-label="LCARS Computer Core">
      <header className="computer-core-header">
        <div><small>LCARS 31.2 DEVELOPMENT · LOCAL-FIRST COMMAND PROCESSOR</small><h2>COMPUTER CORE</h2><p>Translate operator language into visible, permission-aware plans before anything changes.</p></div>
      ...
      <footer className="computer-core-footer"><span>LOCAL-FIRST · EXPLICIT AUTHORITY · DRY-RUN CAPABLE · AUDITABLE</span><small>31.2 OPERATOR AUTHORITY</small></footer>
    </section>
  </div>;
}

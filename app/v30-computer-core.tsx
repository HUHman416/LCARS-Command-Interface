...
export function ComputerCoreConsole({
  ...
  execute: (plan: ComputerPlan, dryRun: boolean) => Promise<void>;
  undo: () => void;
  openBuilder: () => void;
  clearAudit: () => void;
  close: () => void;
  ...
  return <div className="backdrop computer-core-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !running && close()}>
    <section className="computer-core-console" role="dialog" aria-modal="true" aria-label="LCARS Computer Core">
      <header className="computer-core-header">
        <div><small>LCARS 30.1 DEVELOPMENT · LOCAL-FIRST COMMAND PROCESSOR</small><h2>COMPUTER CORE</h2><p>Translate operator language into visible, permission-aware plans before anything changes.</p></div>
      ...
      <footer className="computer-core-footer"><span>LOCAL-FIRST · EXPLICIT AUTHORITY · DRY-RUN CAPABLE · AUDITABLE</span><small>30.1 COMPUTER CORE FOUNDATION</small></footer>
    </section>
  </div>;
}

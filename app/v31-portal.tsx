...
export function PortalCenter({ bridge, operatorName, authority = "", notify, onPendingChange }: Props) {
  ...
  if (!bridge) return <section className="portal-center portal-offline"><header><span><small>VERSION 31.4 · INTENT BROKER</small><h3>LCARS PORTAL CENTER</h3></span><strong>OFFLINE</strong></header><div><b>LOCAL CORE REQUIRED</b><p>The Portal Center is available in the installed LCARS desktop application. Hosted preview mode never receives host files, devices, or permissions.</p></div></section>;
  return <section className="portal-center">
    <header><span><small>VERSION 31.4 DEVELOPMENT · TRUSTED INTENT ROUTING</small><h3>LCARS PORTAL CENTER</h3><p>One operator-controlled route for file choices, device access, protected actions, notifications, printing, and sharing.</p></span><strong className={status?.pending ? "attention" : ""}>{String(status?.pending || 0).padStart(2, "0")}<small>PENDING</small></strong></header>

...
export function PaddLinkPanel({status,busy,refresh,operate}:{status:PaddStatus|null;busy:string;refresh:()=>void;operate:(operation:"start"|"disable"|"revoke"|"role",device?:PaddDevice,role?:PaddRole)=>void}){
  ...
  return <section className="padd-link-panel">
    ...
    <nav><button disabled={busy==="start"||!status?.online} onClick={()=>operate("start")}>{status?.pairing?"REPLACE PAIRING CODE":"ARM PAIRING CODE"}</button><button disabled={busy==="disable"||!status?.enabled} onClick={()=>operate("disable")}>DISABLE PADD LINK</button><button disabled={busy==="refresh"} onClick={refresh}>REFRESH LINK</button></nav>

...
const LCARS_VERSION="29.1.0-dev.1";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setFirstRun(true);
    if(setupComplete&&!safeBoot&&!launchParams.get("tool")&&!localStorage.getItem("lcars-whats-new-v29-1"))setWhatsNewOpen(true);
    setClock(new Date());
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>fetch("http://127.0.0.1:8765/api/padd-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      release:{stable:"28",development:"29.1",channel:prefs.updateChannel},
    })}).catch(()=>{});
  ...
  return (
    ...
    >
            ...
            <section className="detail-view settings-view">
              ...
              />
                ...
                <button onClick={()=>setWhatsNewOpen(true)}>
                  <b>WHAT'S NEW IN VERSION 29.1</b>
      ...
      {whatsNewOpen&&<Version29Welcome close={()=>{localStorage.setItem("lcars-whats-new-v29-1","1");setWhatsNewOpen(false);}} openConnected={()=>{localStorage.setItem("lcars-whats-new-v29-1","1");setWhatsNewOpen(false);setSettingsArea("connected");setSection("settings");}}/>}
      {paletteOpen && (
        <CommandPalette
          query={paletteQuery}
...
function Version29Welcome({close,openConnected}:{close:()=>void;openConnected:()=>void}){
  const features=[
    {code:"01",title:"OPTIONAL ANDROID HOME",text:"Version 29 begins a genuine standalone LCARS Home environment while the paired PADD Companion remains available at any time."},
    {code:"02",title:"APPLICATION LIBRARY",text:"Search and launch installed Android applications through the platform launcher service, including accessible user profiles."},
    {code:"03",title:"FAVORITE APPLICATIONS",text:"Pin frequently used applications to an LCARS command deck and change the selection with explicit star controls or a long press."},
    {code:"04",title:"OFFLINE DEVICE STATUS",text:"Battery, network, storage, and application status remain useful even when no desktop station is paired or online."},
    {code:"05",title:"SAFE HOME SWITCHING",text:"Preview the launcher first, ask Android to make LCARS the Home app only when ready, and reopen system Home settings at any time."},
    {code:"06",title:"DESKTOP WORKSPACE REPAIRS",text:"Wrapped Status module controls remain reachable, and Page Peeks, Communications, and tray windows resize vertically across their full safe range."},
  ];
  return <div className="backdrop whats-new-backdrop"><section className="whats-new-v26" role="dialog" aria-modal="true" aria-label="What.s new in LCARS Version 29.1"><header><span><small>MOBILE COMMAND ENVIRONMENT ORIENTATION</small><h2>WELCOME TO VERSION 29.1</h2><p>Your existing settings and paired PADDs remain in place while Android gains an optional standalone LCARS Home environment.</p></span><strong>29.1</strong></header><div>{features.map((feature)=><article key={feature.code}><i>{feature.code}</i><span><b>{feature.title}</b><p>{feature.text}</p></span></article>)}</div><footer><button onClick={openConnected}>OPEN FLEET COMMAND</button><button autoFocus onClick={close}>START USING VERSION 29.1</button></footer></section></div>;
}
    ...
    <>
      ...
      {historyOpen && (
        <ResizablePopup as="aside" popupKey="communications-center" className="notice-history" floating minWidth={380} minHeight={360} ariaModal={false} ariaLabel="Communications Center">
          <header>
            <div>
              <small>VERSION 29.1 PRIORITY & ACTION MATRIX</small>

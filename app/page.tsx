...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setFirstRun(true);
    if(setupComplete&&!safeBoot&&!launchParams.get("tool")&&!localStorage.getItem("lcars-whats-new-v28"))setWhatsNewOpen(true);
    setClock(new Date());
  ...
  return (
    ...
    >
      ...
      {whatsNewOpen&&<Version28Welcome close={()=>{localStorage.setItem("lcars-whats-new-v28","1");setWhatsNewOpen(false);}} openConnected={()=>{localStorage.setItem("lcars-whats-new-v28","1");setWhatsNewOpen(false);setSettingsArea("connected");setSection("settings");}}/>}
      {paletteOpen && (
        <CommandPalette
          query={paletteQuery}
...
function Version28Welcome({close,openConnected}:{close:()=>void;openConnected:()=>void}){
  const features=[
    {code:"01",title:"PADD FLEET COMMAND",text:"See every paired PADD's live battery, network, latency, role, permissions, layout, Workstation, and connection history from one console."},
    {code:"02",title:"SAFE REMOTE OPERATIONS",text:"Sensitive routines, applications, Workstation restores, handoffs, and clipboard requests wait for explicit desktop approval unless you grant a trusted-device override."},
    {code:"03",title:"COMMUNICATIONS & HANDOFF",text:"Acknowledge or archive notices, launch quick actions, follow active routines, and hand the current desktop console to a phone or tablet."},
    {code:"04",title:"NATIVE ANDROID PADD",text:"Use the new tablet and landscape layouts, home-screen status widget, priority notifications, identify haptics, accessibility sync, and live device telemetry."},
    {code:"05",title:"PROXIMITY WORKSTATIONS",text:"Optionally restore a chosen Workstation when a trusted PADD arrives and return to your default profile when it disconnects."},
    {code:"06",title:"CONNECTED DIAGNOSTICS",text:"Review the persistent command journal, approval queue, release matrix, and connection diagnostics without exposing terminal, files, processes, or power controls."},
  ];
  return <div className="backdrop whats-new-backdrop"><section className="whats-new-v26" role="dialog" aria-modal="true" aria-label="What's new in LCARS Version 28"><header><span><small>CONNECTED OPERATIONS ORIENTATION</small><h2>WELCOME TO VERSION 28</h2><p>Your existing settings and paired PADDs remain in place while the connected fleet gains safer, richer control.</p></span><strong>28</strong></header><div>{features.map((feature)=><article key={feature.code}><i>{feature.code}</i><span><b>{feature.title}</b><p>{feature.text}</p></span></article>)}</div><footer><button onClick={openConnected}>OPEN FLEET COMMAND</button><button autoFocus onClick={close}>START USING VERSION 28</button></footer></section></div>;
}
    ...
    <>
      ...
      {historyOpen && (
        <ResizablePopup as="aside" popupKey="communications-center" className="notice-history" floating minWidth={380} minHeight={360} ariaModal={false} ariaLabel="Communications Center">
          <header>
            <div>
              <small>VERSION 28 PRIORITY & ACTION MATRIX</small>

...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>fetch("http://127.0.0.1:8765/api/padd-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      media:players.map((player)=>({id:player.id,name:player.name,title:player.title,artist:player.artist,album:player.album,artUrl:player.artUrl||player.icon||"",position:player.position||0,length:player.length||0,detail:`${player.artist||"UNKNOWN ARTIST"} · ${player.status||"UNKNOWN"}`,status:player.status})),
      meters:meters.map((meter)=>({label:meter[0],value:meter[1],detail:meter[2]})),
      routines:routines.filter((routine)=>routine.enabled).map((routine)=>({id:routine.id,name:routine.name,detail:`${routine.steps.length} STEPS`})),
      ...
      apps:favorites.map((app)=>({id:app.id,name:app.name,detail:app.comment})),
      notices:notices.filter((notice)=>!notice.archived).slice(0,24).map((notice)=>({id:String(notice.id),name:notice.source||"LCARS CORE",text:notice.text,detail:notice.text,status:(notice.priority||notice.kind).toUpperCase(),priority:notice.priority||notice.kind,read:Boolean(notice.read)})),
  ...
  },[bridge,section,theme,volume,audioMuted,doNotDisturb,players,meters,routines,favorites,notices,apps,profiles,activeProfile,runningRoutine,access,prefs.updateChannel,defaultWorkstation]);
  useEffect(()=>{

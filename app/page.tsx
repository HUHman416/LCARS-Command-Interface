...
const speedDialChoices: { id: SpeedDialItem; label: string; description: string }[] = [
  ...
  { id:"action:computer", label:"COMPUTER", description:"Open Version 30 Computer Core" },
];
...
export default function Home() {
  ...
  const runCommissioningCheck=async()=>{
    ...
    recordActivity("Commissioning check",`${passed}/${results.length} service groups answered`,passed===results.length?"success":"attention","SYSTEM",false,{subsystem:"SYSTEM",group:"commissioning:check",explanation:"Version 30 refreshed local integration, Federation, Data Fabric, native controls, and release-channel readiness together."});
  ...
  useEffect(()=>{
    const frame=window.requestAnimationFrame(()=>{
      document.querySelector<HTMLElement>(".lcars")?.scrollTo(0,0);
        ...
        <div className="title">
          <div className="title-copy">
            <div className={`title-kicker${prefs.voiceEnabled?" voice-active":""}`}>
              ...
              <button className={`header-operator role-${activeOperator?.role||"guest"}`} onClick={()=>setOperatorCenterOpen(true)} title="Switch operator or manage Version 30 identities"><i>{activeOperator?.awayTeam?"AT":activeOperator?.role==="administrator"?"A":activeOperator?.role==="operator"?"O":"G"}</i><span>{activeOperator?.name||userName}</span></button>
        ...
        <section className={"content page-" + section}>
          ...
          {section === "system" && (
            <section className="detail-view systems-command-page">
              <SystemControlCenter key={systemControlArea} initialArea={systemControlArea} platform={platform} embedded notify={notify} openPage={setSection} openPortal={()=>setSection("portals")} openApplications={()=>setAllOpen(true)} openFiles={(path)=>{setFileBrowseTarget(path);setSection("files");}} prepareTerminal={(command)=>{setTerminalSeed(command);setSection("terminal");}}/>
            </section>
          )}
          {section === "media" && (
...
function ExtensionHub({installed,catalog,sources,setCatalog,setSources,disabled,setDisabled,refresh,notify,openFolder,openByDefault=false}:{installed:ExtensionManifest[];catalog:ExtensionCatalogEntry[];sources:ModuleRepositorySource[];setCatalog:(items:ExtensionCatalogEntry[])=>void;setSources:(items:ModuleRepositorySource[])=>void;disabled:string[];setDisabled:(ids:string[])=>void;refresh:()=>void;notify:(text:string,kind?:"info"|"error")=>void;openFolder:()=>void;openByDefault?:boolean}){
  ...
  return <section className={`extension-hub module-repository-panel ${expanded?"repository-open":"repository-closed"}`}>
    ...
    {expanded&&<>
      ...
      <footer><b>VERSION 30 MODULE SAFETY</b> · Signed API v3 packages, explicit capability grants, per-module health records, automatic render isolation, two-version rollback, bounded portable imports, and channel-aware repositories. Repository code is never executed.</footer>
    </>}
  </section>;
}

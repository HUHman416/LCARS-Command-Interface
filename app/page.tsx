...
const speedDialChoices: { id: SpeedDialItem; label: string; description: string }[] = [
  ...
  { id:"action:computer", label:"COMPUTER", description:"Open Version 30.14 Computer Core" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setFirstRun(true);
    if(setupComplete&&!safeBoot&&!launchParams.get("tool")&&!localStorage.getItem("lcars-whats-new-v30-13"))setWhatsNewOpen(true);
    setClock(new Date());
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>fetch("http://127.0.0.1:8765/api/padd-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      release:{stable:"29",development:"30.14",channel:prefs.updateChannel},
    })}).catch(()=>{});
  ...
  const runCommissioningCheck=async()=>{
    ...
    recordActivity("Commissioning check",`${passed}/${results.length} service groups answered`,passed===results.length?"success":"attention","SYSTEM",false,{subsystem:"SYSTEM",group:"commissioning:check",explanation:"Version 30.14 refreshed local integration, Federation, Data Fabric, native controls, and release-channel readiness together."});
  ...
  return (
    ...
    >
            ...
            <section className="detail-view settings-view">
              ...
              />
                ...
                <button onClick={()=>setWhatsNewOpen(true)}>
                  <b>WHAT&apos;S NEW IN VERSION 30.14</b>
      ...
      {whatsNewOpen&&<Version30Welcome close={()=>{localStorage.setItem("lcars-whats-new-v30-13","1");setWhatsNewOpen(false);}} openCommissioning={()=>{localStorage.setItem("lcars-whats-new-v30-13","1");setWhatsNewOpen(false);setSystemControlArea("telemetry");setSection("system");}}/>}
...
function ExtensionHub({installed,catalog,sources,setCatalog,setSources,disabled,setDisabled,refresh,notify,openFolder,openByDefault=false}:{installed:ExtensionManifest[];catalog:ExtensionCatalogEntry[];sources:ModuleRepositorySource[];setCatalog:(items:ExtensionCatalogEntry[])=>void;setSources:(items:ModuleRepositorySource[])=>void;disabled:string[];setDisabled:(ids:string[])=>void;refresh:()=>void;notify:(text:string,kind?:"info"|"error")=>void;openFolder:()=>void;openByDefault?:boolean}){
  ...
  return <section className={`extension-hub module-repository-panel ${expanded?"repository-open":"repository-closed"}`}>
    ...
    {expanded&&<>
      ...
      <footer><b>VERSION 30.14 MODULE SAFETY</b> · Signed API v3 packages, explicit capability grants, per-module health records, automatic render isolation, two-version rollback, bounded portable imports, and channel-aware repositories. Repository code is never executed.</footer>
    </>}
  </section>;
}
...
function OperatorCenter({operators,activeId,devices,canManage,close,switchOperator,createOperator,updateOperator,setPin,deleteOperator,exportOperator,importOperator,saveStationPreference,roamOperator}:{
  ...
  return <div className="backdrop operator-center-backdrop" onMouseDown={(change)=>change.target===change.currentTarget&&close()}><ResizablePopup popupKey="operator-center" className="operator-center" ariaModal={true} minWidth={760} minHeight={560}>
    <header><div><small>VERSION 30.14 · INDIVIDUAL COMMAND ENVIRONMENTS</small><h2>OPERATOR IDENTITIES</h2><p>Switch complete favorites, decks, Display Matrix, layouts, commands, and station preferences without mixing operator configurations.</p></div><button onClick={close}>CLOSE ×</button></header>
...
function Version30Welcome({close,openCommissioning}:{close:()=>void;openCommissioning:()=>void}){
  const features=[
    {code:"01",title:"CLEANER CONTROL DECK",text:"Each of the eleven Systems tabs now shows one clear label, with full names on desktop and compact codes only when needed."},
    {code:"02",title:"DELIBERATE LCARS GEOMETRY",text:"The awkward masthead curve is replaced by a square junction and coordinated previous and next end caps."},
    {code:"03",title:"NO MORE CUT-OFF PANELS",text:"Systems can extend safely inside the desktop shell, with visible scrolling and clearance above the command tray."},
    {code:"04",title:"HORIZONTAL RECORD LAYOUTS",text:"Connectivity, audio, displays, processes, storage, and modules distribute cards across available columns."},
    {code:"05",title:"COMPACT TELEMETRY",text:"Denser processor rows and responsive hardware banks keep more live information on screen at once."},
    {code:"06",title:"ELEVEN-PANEL AUDIT",text:"Every Systems section now shares the same overflow, spacing, readable-label, and responsive-fit safeguards."},
  ];
  return <div className="backdrop whats-new-backdrop"><section className="whats-new-v26" role="dialog" aria-modal="true" aria-label="What's new in LCARS Version 30.14 Development"><header><span><small>FEDERATION OPERATING ENVIRONMENT · DEVELOPMENT</small><h2>VERSION 30.14 · SYSTEMS LAYOUT AUDIT</h2><p>A cleaner control deck and complete responsive-fit pass for all eleven Systems stations.</p></span><strong>30</strong></header><div>{features.map((feature)=><article key={feature.code}><i>{feature.code}</i><span><b>{feature.title}</b><p>{feature.text}</p></span></article>)}</div><footer><button onClick={openCommissioning}>OPEN SYSTEMS PAGE</button><button autoFocus onClick={close}>BEGIN 30.14 DEVELOPMENT</button></footer></section></div>;
}

...
const speedDialChoices: { id: SpeedDialItem; label: string; description: string }[] = [
  ...
  { id:"action:computer", label:"COMPUTER", description:"Open Version 30.13 Computer Core" },
];
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>fetch("http://127.0.0.1:8765/api/padd-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      release:{stable:"29",development:"30.13",channel:prefs.updateChannel},
    })}).catch(()=>{});
  ...
  const runCommissioningCheck=async()=>{
    ...
    recordActivity("Commissioning check",`${passed}/${results.length} service groups answered`,passed===results.length?"success":"attention","SYSTEM",false,{subsystem:"SYSTEM",group:"commissioning:check",explanation:"Version 30.13 refreshed local integration, Federation, Data Fabric, native controls, and release-channel readiness together."});
            ...
            <section className="detail-view settings-view">
              ...
              />
                ...
                <button onClick={()=>setWhatsNewOpen(true)}>
                  <b>WHAT&apos;S NEW IN VERSION 30.13</b>
...
function ExtensionHub({installed,catalog,sources,setCatalog,setSources,disabled,setDisabled,refresh,notify,openFolder,openByDefault=false}:{installed:ExtensionManifest[];catalog:ExtensionCatalogEntry[];sources:ModuleRepositorySource[];setCatalog:(items:ExtensionCatalogEntry[])=>void;setSources:(items:ModuleRepositorySource[])=>void;disabled:string[];setDisabled:(ids:string[])=>void;refresh:()=>void;notify:(text:string,kind?:"info"|"error")=>void;openFolder:()=>void;openByDefault?:boolean}){
  ...
  return <section className={`extension-hub module-repository-panel ${expanded?"repository-open":"repository-closed"}`}>
    ...
    {expanded&&<>
      ...
      <footer><b>VERSION 30.13 MODULE SAFETY</b> · Signed API v3 packages, explicit capability grants, per-module health records, automatic render isolation, two-version rollback, bounded portable imports, and channel-aware repositories. Repository code is never executed.</footer>
    </>}
  </section>;
}
...
function OperatorCenter({operators,activeId,devices,canManage,close,switchOperator,createOperator,updateOperator,setPin,deleteOperator,exportOperator,importOperator,saveStationPreference,roamOperator}:{
  ...
  return <div className="backdrop operator-center-backdrop" onMouseDown={(change)=>change.target===change.currentTarget&&close()}><ResizablePopup popupKey="operator-center" className="operator-center" ariaModal={true} minWidth={760} minHeight={560}>
    <header><div><small>VERSION 30.13 · INDIVIDUAL COMMAND ENVIRONMENTS</small><h2>OPERATOR IDENTITIES</h2><p>Switch complete favorites, decks, Display Matrix, layouts, commands, and station preferences without mixing operator configurations.</p></div><button onClick={close}>CLOSE ×</button></header>
...
function Version30Welcome({close,openCommissioning}:{close:()=>void;openCommissioning:()=>void}){
  const features=[
    {code:"01",title:"ONE HORIZONTAL CONTROL DECK",text:"All eleven Systems sections now remain on a single LCARS rail instead of stacking into vertical rows."},
    {code:"02",title:"EVERY SECTION REACHABLE",text:"Previous and next controls, touch scrolling, arrow keys, and automatic active-tab reveal prevent hidden sections."},
    {code:"03",title:"ADAPTIVE LABELS",text:"Full names appear when space permits; compact section codes take over before the rail needs to scroll."},
    {code:"04",title:"LESS VERTICAL TRAVEL",text:"A shorter masthead and toolbar return more of the screen to live system information."},
    {code:"05",title:"DENSER TELEMETRY",text:"Meters and hardware records fill horizontal space first and keep two columns at ordinary compact-desktop widths."},
    {code:"06",title:"UNIFIED SYSTEMS RETAINED",text:"Every Version 30.12 telemetry surface and Version 30.11 native control remains in the embedded Systems page."},
  ];
  return <div className="backdrop whats-new-backdrop"><section className="whats-new-v26" role="dialog" aria-modal="true" aria-label="What's new in LCARS Version 30.13 Development"><header><span><small>FEDERATION OPERATING ENVIRONMENT · DEVELOPMENT</small><h2>VERSION 30.13 · HORIZONTAL SYSTEMS NAVIGATION</h2><p>Every Systems station stays close at hand while Telemetry uses more of the available screen.</p></span><strong>30</strong></header><div>{features.map((feature)=><article key={feature.code}><i>{feature.code}</i><span><b>{feature.title}</b><p>{feature.text}</p></span></article>)}</div><footer><button onClick={openCommissioning}>OPEN SYSTEMS PAGE</button><button autoFocus onClick={close}>BEGIN 30.13 DEVELOPMENT</button></footer></section></div>;
}

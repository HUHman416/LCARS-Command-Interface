...
const LCARS_VERSION="29.2.0-dev.1";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setFirstRun(true);
    if(setupComplete&&!safeBoot&&!launchParams.get("tool")&&!localStorage.getItem("lcars-whats-new-v29-2"))setWhatsNewOpen(true);
    setClock(new Date());
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>fetch("http://127.0.0.1:8765/api/padd-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      release:{stable:"28",development:"29.2",channel:prefs.updateChannel},
    })}).catch(()=>{});
  ...
  return (
    ...
    >
            ...
            <section className="detail-view settings-view">
              ...
              />
                ...
                <button onClick={()=>setWhatsNewOpen(true)}>
                  <b>WHAT'S NEW IN VERSION 29.2</b>
      ...
      {whatsNewOpen&&<Version29Welcome close={()=>{localStorage.setItem("lcars-whats-new-v29-2","1");setWhatsNewOpen(false);}} openConnected={()=>{localStorage.setItem("lcars-whats-new-v29-2","1");setWhatsNewOpen(false);setSettingsArea("connected");setSection("settings");}}/>}
      {paletteOpen && (
        <CommandPalette
          query={paletteQuery}
...
function Version29Welcome({close,openConnected}:{close:()=>void;openConnected:()=>void}){
  const features=[
    {code:"01",title:"DESKTOP-STYLE MOBILE SHELL",text:"Standalone Home now uses the same LCARS masthead, left navigation rail, numbered pages, and focused content model as the desktop interface."},
    {code:"02",title:"DECKS & FOLDERS",text:"Create operator decks, group installed applications into folders, and manage Favorites independently from either collection."},
    {code:"03",title:"NATIVE ANDROID WIDGETS",text:"Select, configure, host, and remove installed Android widgets through the platform widget picker."},
    {code:"04",title:"MOBILE DISPLAY MATRIX",text:"Switch among six verified LCARS families with distinct palette, corner geometry, borders, navigation, and density."},
    {code:"05",title:"LAYOUT & BACKUP",text:"Tune sidebar width, density, and application columns, then export or restore portable Home configuration files."},
    {code:"06",title:"RESILIENT MOBILE HEADERS",text:"The Companion and standalone Home mastheads now fit safely within their panels after navigation, accessibility scaling, rotation, and return to Status."},
  ];
  return <div className="backdrop whats-new-backdrop"><section className="whats-new-v26" role="dialog" aria-modal="true" aria-label="What's new in LCARS Version 29.2"><header><span><small>MOBILE COMMAND ENVIRONMENT ORIENTATION</small><h2>WELCOME TO VERSION 29.2</h2><p>The complete launcher-customization milestone is ready while all Version 29.1 Home and paired PADD capabilities remain in place.</p></span><strong>29.2</strong></header><div>{features.map((feature)=><article key={feature.code}><i>{feature.code}</i><span><b>{feature.title}</b><p>{feature.text}</p></span></article>)}</div><footer><button onClick={openConnected}>OPEN FLEET COMMAND</button><button autoFocus onClick={close}>START USING VERSION 29.2</button></footer></section></div>;
}
    ...
    <>
      ...
      {historyOpen && (
        <ResizablePopup as="aside" popupKey="communications-center" className="notice-history" floating minWidth={380} minHeight={360} ariaModal={false} ariaLabel="Communications Center">
          <header>
            <div>
              <small>VERSION 29.2 PRIORITY & ACTION MATRIX</small>

...
const LCARS_VERSION="29.3.0-rc.1";
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    const synchronize=()=>fetch("http://127.0.0.1:8765/api/padd-sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      ...
      release:{stable:"28",development:"29.3",channel:prefs.updateChannel},
    })}).catch(()=>{});
            ...
            <section className="detail-view settings-view">
              ...
              />
                ...
                <button onClick={()=>setWhatsNewOpen(true)}>
                  <b>WHAT'S NEW IN VERSION 29.3 RC 1</b>
...
function Version29Welcome({close,openConnected}:{close:()=>void;openConnected:()=>void}){
  ...
  return <div className="backdrop whats-new-backdrop"><section className="whats-new-v26" role="dialog" aria-modal="true" aria-label="What's new in LCARS Version 29.3 RC 1"><header><span><small>MOBILE COMMAND ENVIRONMENT RELEASE CANDIDATE</small><h2>WELCOME TO VERSION 29.3 RC 1</h2><p>The complete Version 29 feature set is assembled for phone, tablet, landscape, upgrade, battery, and signing validation.</p></span><strong>29.3</strong></header><div>{features.map((feature)=><article key={feature.code}><i>{feature.code}</i><span><b>{feature.title}</b><p>{feature.text}</p></span></article>)}</div><footer><button onClick={openConnected}>OPEN FLEET COMMAND</button><button autoFocus onClick={close}>START USING VERSION 29.3</button></footer></section></div>;
}
    ...
    <>
      ...
      {historyOpen && (
        <ResizablePopup as="aside" popupKey="communications-center" className="notice-history" floating minWidth={380} minHeight={360} ariaModal={false} ariaLabel="Communications Center">
          <header>
            <div>
              <small>VERSION 29.3 PRIORITY & ACTION MATRIX</small>

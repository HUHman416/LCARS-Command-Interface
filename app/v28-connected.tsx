...
export function ConnectedOperationsPanel({status,busy,now,workstations,refresh,operate}:{
  ...
  return <section className="connected-operations-panel">
    <header className="connected-operations-head">
      <span><small>VERSION 29 STABLE · CONNECTED STATION DOCK</small><b>PADD FLEET COMMAND</b><p>One coordinated LCARS environment across desktop and trusted PADDs, with encrypted multi-station mobile credentials, per-device permissions, live state, approvals, notifications, diagnostics, and Workstation handoff.</p></span>
    ...
    {area==="fleet"&&<>
      <section className="padd-pairing-deck">
        ...
        <div><a className="padd-download-v28" href="https://github.com/HUHman416/LCARS-Command-Interface/releases/download/v29/LCARS-Mobile-Environment-v29-Android.apk" target="_blank" rel="noreferrer">DOWNLOAD ANDROID 29 STABLE ↗</a><button disabled={busy==="start"} onClick={()=>operate("start")}>{status?.pairing?"REPLACE CODE":"ARM ONE-USE CODE"}</button><button disabled={!primaryAddress} onClick={copySetup}>{copied?"SETUP COPIED":"COPY STATION + CODE"}</button><button disabled={busy==="refresh"} onClick={refresh}>REFRESH LINK</button><button className="danger" disabled={busy==="disable"||!status?.enabled} onClick={()=>operate("disable")}>DISABLE LINK</button></div>

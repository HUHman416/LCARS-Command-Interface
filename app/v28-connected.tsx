...
export type PaddDevice = {
  ...
  lastAddress?:string;connectionCount?:number;battery?:number;network?:string;latencyMs?:number;clientVersion?:string;continuumRole?:string;
...
export function ConnectedOperationsPanel({status,busy,now,workstations,refresh,operate}:{
  ...
  return <section className="connected-operations-panel">
    <header className="connected-operations-head">
      <span><small>VERSION 30.8 DEVELOPMENT · CONTINUUM</small><b>FEDERATION OPERATIONS</b><p>One operator, many trusted stations: automatic discovery, durable identities, adaptive device roles, AES-256-GCM synchronization, selective data routes, operational handoff, and bounded offline delivery queues.</p></span>
    ...
    {area==="fleet"&&<>
      <section className="padd-pairing-deck">
        ...
        <div><a className="padd-download-v28" href="https://github.com/HUHman416/LCARS-Command-Interface/releases/download/v30.8.1/LCARS-Mobile-Environment-v30.8.1-Android.apk" target="_blank" rel="noreferrer">DOWNLOAD ANDROID 30.8.1 ↗</a><button disabled={busy==="start"} onClick={()=>operate("start")}>{status?.pairing?"REPLACE CODE":"ARM ONE-USE CODE"}</button><button disabled={!primaryAddress} onClick={copySetup}>{copied?"SETUP COPIED":"COPY STATION + CODE"}</button><button disabled={busy==="refresh"} onClick={refresh}>REFRESH LINK</button><button className="danger" disabled={busy==="disable"||!status?.enabled} onClick={()=>operate("disable")}>DISABLE LINK</button></div>
      ...
      <section className="padd-fleet-list">
        ...
        {status?.devices.length?status.devices.map((device,index)=>{
          ...
          return <article className={device.online?"online":""} key={device.id}>
            ...
            <footer><span>{device.lastAddress||"ADDRESS NOT RECORDED"} · {(device.continuumRole||"HANDHELD HOME").replaceAll("-"," ").toUpperCase()} · {device.network||"NETWORK UNKNOWN"} · {device.latencyMs||0} MS · {device.connectionCount||0} LINKS · CLIENT {device.clientVersion||"UNKNOWN"}</span><div><button onClick={()=>operate("identify",device)}>IDENTIFY</button><button className="danger" onClick={()=>operate("revoke",device)}>REVOKE</button></div></footer>
          </article>;

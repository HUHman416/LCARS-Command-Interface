...
export type PaddPermission = "navigate" | "media" | "volume" | "dnd" | "routine" | "app" | "workstation" | "notice-read" | "notice-archive" | "quick" | "handoff" | "clipboard" | "autoApprove" | "communications" | "telemetry";
...
export type PaddApproval = {id:string;action:string;value:unknown;device:string;deviceName:string;createdAt:number};
...
export type PaddOperation = "start" | "disable" | "revoke" | "role" | "rename" | "permissions" | "profile" | "layout" | "proximity" | "identify" | "clipboard" | "approve" | "deny";
...
const permissionGroups:{title:string;items:{id:PaddPermission;label:string}[]}[]=[
  ...
  {title:"CONNECTED OPERATIONS",items:[{id:"communications",label:"COMMUNICATIONS"},{id:"telemetry",label:"TELEMETRY"},{id:"notice-read",label:"ACKNOWLEDGE"},{id:"notice-archive",label:"ARCHIVE"},{id:"handoff",label:"HANDOFF"}]},
...
const roleDefaults:Record<PaddRole,Partial<Record<PaddPermission,boolean>>>={
  ...
  operator:{navigate:true,media:true,volume:true,quick:true,communications:true,telemetry:true,"notice-read":true,"notice-archive":true,handoff:true},
  command:{navigate:true,media:true,volume:true,quick:true,communications:true,telemetry:true,"notice-read":true,"notice-archive":true,handoff:true,dnd:true,routine:true,app:true,workstation:true,clipboard:true,autoApprove:false},
};
...
export function ConnectedOperationsPanel({status,busy,now,workstations,refresh,operate}:{
  ...
  return <section className="connected-operations-panel">
    <header className="connected-operations-head">
      <span><small>VERSION 28 · CONNECTED OPERATIONS</small><b>PADD FLEET COMMAND</b><p>One coordinated LCARS environment across desktop and trusted PADDs, with per-device permissions, live state, approvals, diagnostics, and Workstation handoff.</p></span>
    ...
    {area==="fleet"&&<>
      <section className="padd-pairing-deck">
        ...
        <div><a className="padd-download-v28" href="https://github.com/HUHman416/LCARS-Command-Interface/releases/download/v28.1/LCARS-PADD-Companion-v28.1-Android.apk" target="_blank" rel="noreferrer">DOWNLOAD ANDROID PADD ↗</a><button disabled={busy==="start"} onClick={()=>operate("start")}>{status?.pairing?"REPLACE CODE":"ARM ONE-USE CODE"}</button><button disabled={!primaryAddress} onClick={copySetup}>{copied?"SETUP COPIED":"COPY STATION + CODE"}</button><button disabled={busy==="refresh"} onClick={refresh}>REFRESH LINK</button><button className="danger" disabled={busy==="disable"||!status?.enabled} onClick={()=>operate("disable")}>DISABLE LINK</button></div>
      ...
      <section className="padd-fleet-list">
        ...
        {status?.devices.length?status.devices.map((device,index)=>{
          ...
          return <article className={device.online?"online":""} key={device.id}>
            ...
            <footer><span>{device.lastAddress||"ADDRESS NOT RECORDED"} · {device.network||"NETWORK UNKNOWN"} · {device.latencyMs||0} MS · {device.connectionCount||0} LINKS</span><div><button onClick={()=>operate("identify",device)}>IDENTIFY</button><button className="danger" onClick={()=>operate("revoke",device)}>REVOKE</button></div></footer>
          </article>;
    ...
    {area==="approvals"&&<section className="padd-approval-list">
      ...
      {status?.approvals?.length?status.approvals.map((approval,index)=><article key={approval.id}><i>{String(index+1).padStart(2,"0")}</i><span><small>{approval.deviceName} · {age(approval.createdAt)}</small><b>{approval.action.toUpperCase()}</b><p>{displayValue(approval.value)}</p></span><div><button onClick={()=>operate("approve",undefined,{approvalId:approval.id})}>APPROVE</button><button className="danger" onClick={()=>operate("deny",undefined,{approvalId:approval.id})}>DENY</button></div></article>):<p className="connected-empty">APPROVAL QUEUE CLEAR</p>}
    </section>}
    ...
    {area==="diagnostics"&&<section className="padd-diagnostics-v28">
      ...
      <div className="padd-diagnostic-actions"><button onClick={refresh}>RUN LINK CHECK</button><button disabled={!primaryAddress} onClick={copySetup}>COPY CONNECTION DETAILS</button></div>
    </section>}
  </section>;
}

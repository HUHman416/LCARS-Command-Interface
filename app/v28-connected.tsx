...
export function ConnectedOperationsPanel({status,busy,now,workstations,refresh,operate}:{
  ...
  return <section className="connected-operations-panel">
    <header className="connected-operations-head">
      <span><small>VERSION 31.3 DEVELOPMENT · CONTINUUM</small><b>FEDERATION OPERATIONS</b><p>One operator, many trusted stations: automatic discovery, durable identities, adaptive device roles, AES-256-GCM synchronization, selective data routes, operational handoff, and bounded offline delivery queues.</p></span>
    ...
    {area==="fleet"&&<>
      <section className="padd-pairing-deck">
        ...
        <div><a className="padd-download-v28" href="https://github.com/HUHman416/LCARS-Command-Interface/releases/download/v30/LCARS-Mobile-Environment-v30-Android.apk" target="_blank" rel="noreferrer">DOWNLOAD ANDROID 30 ↗</a><button disabled={busy==="start"} onClick={()=>operate("start")}>{status?.pairing?"REPLACE CODE":"ARM ONE-USE CODE"}</button><button disabled={!primaryAddress} onClick={copySetup}>{copied?"SETUP COPIED":"COPY STATION + CODE"}</button><button disabled={busy==="refresh"} onClick={refresh}>REFRESH LINK</button><button className="danger" disabled={busy==="disable"||!status?.enabled} onClick={()=>operate("disable")}>DISABLE LINK</button></div>

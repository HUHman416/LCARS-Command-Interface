  ...
  return <section className="connected-operations-panel">
    ...
    {area==="fleet"&&<>
      <section className="padd-pairing-deck">
        ...
        <div><a className="padd-download-v28" href="https://github.com/HUHman416/LCARS-Command-Interface/releases/download/v30.8/LCARS-Mobile-Environment-v30.8-Android.apk" target="_blank" rel="noreferrer">DOWNLOAD ANDROID 30.8 ↗</a><button disabled={busy==="start"} onClick={()=>operate("start")}>{status?.pairing?"REPLACE CODE":"ARM ONE-USE CODE"}</button><button disabled={!primaryAddress} onClick={copySetup}>{copied?"SETUP COPIED":"COPY STATION + CODE"}</button><button disabled={busy==="refresh"} onClick={refresh}>REFRESH LINK</button><button className="danger" disabled={busy==="disable"||!status?.enabled} onClick={()=>operate("disable")}>DISABLE LINK</button></div>

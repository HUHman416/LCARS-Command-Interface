...
class PaddPairingTests(unittest.TestCase):
    ...
    def test_sensitive_deliveries_require_global_and_per_station_opt_in(self):
        with tempfile.TemporaryDirectory() as folder:
            controller = self.make_controller(folder)
            armed = controller.manage({"operation": "start"})
            paired = controller.pair(armed["pairing"]["code"], "Command PADD", "192.168.1.42")
            ident = paired["device"]["id"]
            controller.manage({"operation": "role", "id": ident, "role": "command"})
            controller.manage({"operation": "clipboard", "enabled": True})
            with self.assertRaisesRegex(PermissionError, "this station"):
                controller.manage({"operation": "delivery", "id": ident, "kind": "clipboard", "payload": {"text": "alpha"}})
            controller.manage({"operation": "sync-policy", "id": ident, "sync": {"clipboard": True}})
            with self.assertRaisesRegex(PermissionError, "encrypted native Federation link"):
                controller.manage({"operation": "delivery", "id": ident, "kind": "clipboard", "payload": {"text": "alpha"}})

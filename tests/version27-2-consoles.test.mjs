...
test("Version 27.2 uses focused Settings and Updates consoles", () => {
...
test("PADD pairing is guided and Android owns the companion interface", () => {
  ...
  assert.match(connected, /LCARS-PADD-Companion-v27\.2-Android\.apk/);
...
test("Version 27.2 Development release includes all three platforms", () => {
  assert.match(workflow, /branches: \[27\.1\]/);
  assert.match(workflow, /needs: \[linux, windows, android\]/);
  assert.match(workflow, /gh release (?:view|create) v27\.2/);

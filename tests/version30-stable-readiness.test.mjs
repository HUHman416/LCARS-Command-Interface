...
test("Version 31.5 development identity preserves the Version 30 stable release assets",async()=>{
  ...
  assert.match(page,/const LCARS_VERSION="31\.5"/);
  assert.match(padd,/VERSION 31\.5 DEVELOPMENT/);
  assert.match(linux,/LCARS_VERSION="31\.5"/);
  assert.match(windows,/LCARS_VERSION="31\.5"/);

...
test("Version 31.4 development identity preserves the Version 30 stable release assets",async()=>{
  ...
  assert.match(page,/const LCARS_VERSION="31\.4"/);
  assert.match(padd,/VERSION 31\.4 DEVELOPMENT/);
  assert.match(linux,/LCARS_VERSION="31\.4"/);
  assert.match(windows,/LCARS_VERSION="31\.4"/);

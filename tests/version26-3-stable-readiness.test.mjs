...
test("26.3 RC and Version 26 stable identities remain explicit", () => {
  ...
  assert.match(page, /const LCARS_VERSION="(?:30\.2|30\.1-A|29\.0\.0|29\.3\.0-rc\.1|29\.2\.0-dev\.1|28\.0\.0|28\.3-rc\.1|28\.2-dev\.1|27\.(?:2\.[01]|1\.1)-dev\.1|26\.(?:3\.0-dev\.1|0\.0))"/);
...
test("upgraded operators receive a one-time current-version orientation", () => {
  assert.match(page, /lcars-whats-new-v(?:30-[12]|29-[123]|28)/);
  assert.match(page, /(?:VERSION 30\.2 · FEDERATION|VERSION 30\.1 · COMPUTER CORE|WELCOME TO VERSION 29|WELCOME TO VERSION 29\.3 RC 1|WELCOME TO VERSION 29\.2|WELCOME TO VERSION 28)/);
  assert.match(page, /(?:WHAT(?:'|&apos;)S NEW IN VERSION 30\.2|WHAT'S NEW IN VERSION 30\.1|WHAT'S NEW IN VERSION 29|WHAT'S NEW IN VERSION 29\.3 RC 1|WHAT'S NEW IN VERSION 29\.2|WHAT'S NEW IN VERSION 28)/);

...
test("26.3 RC and Version 26 stable identities remain explicit", () => {
  ...
  assert.match(page, /const LCARS_VERSION="(?:27\.1\.1-dev\.1|26\.(?:3\.0-dev\.1|0\.0))"/);
  assert.match(linux, /LCARS_VERSION="(?:27\.1\.1-dev\.1|26\.(?:3\.0-dev\.1|0\.0))"/);
  assert.match(windows, /LCARS_VERSION="(?:27\.1\.1-dev\.1|26\.(?:3\.0-dev\.1|0\.0))"/);

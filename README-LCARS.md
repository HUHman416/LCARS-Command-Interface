...
Version 30.9 is the current Release Candidate and Version 29 remains Stable. The new Commissioning and Trust Center brings integration checks, offline command help, authority, privacy, bounded storage, recovery, and release readiness together. The Browser Station experiment has been removed completely. Every desktop rail destination—including custom pages, Tasks, and Power—uses one continuous number sequence. File Explorer media routing, Android Continuum, Operator Workspaces, Operations Center, Universal Search and Data Fabric, the optional LCARS Session, Module Platform, Federation, and Computer Core remain intact. See `docs/MODULE-API-v3.md` for the public module contract.

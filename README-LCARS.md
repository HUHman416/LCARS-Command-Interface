...
- Version 30.12 unified Systems page with an embedded control matrix and one complete CPU, GPU, memory, disk, hardware, storage, and engineering Telemetry station
- Green Alert and No Alert voice phrases plus distinct spoken single-command and compact multi-command confirmation cues

## Current stage

Version 30.12 is the current Development milestone and Version 29 remains Stable. Systems now opens the System Control Matrix directly in the sidebar page, with complete live telemetry as its first station. The former diagnostic meters, Expanded Hardware Matrix, Storage Matrix, Engineering Console, and hardware pop-ups were consolidated: unique per-core, physical-memory-module, GPU driver, VRAM, temperature, display-mode, storage-topology, and engineering-sensor data remain available without duplicated surfaces. Version 30.11 native controls, Version 30.10 Authoritative Session, and all earlier Version 30 work remain intact. See `docs/VERSION-30.12-UNIFIED-SYSTEMS-AUDIT.md` for the consolidation audit, `docs/VERSION-30.11-NATIVE-CONTROLS-AUDIT.md` for the handoff audit, and `docs/VERSION-30.10-SESSION-AUDIT.md` for the session audit.

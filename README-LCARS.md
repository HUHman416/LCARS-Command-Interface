...
Version 30.10 is the current Development milestone and Version 29 remains Stable. It advances the already-selectable Linux LCARS Session into an explicit authoritative mode: supported host panels are hidden, application windows remain managed by the existing compositor, Task Rail windows can move directly to decks, placement rules can remain continuously enforced, clean exits reveal the preserved desktop, crashes follow bounded recovery, and Meta+Shift+Escape provides a real emergency route. The 30.9 Commissioning and Trust Center and all earlier Version 30 work remain intact. See `docs/VERSION-30.10-SESSION-AUDIT.md` for the deduplication audit and `docs/MODULE-API-v3.md` for the public module contract.

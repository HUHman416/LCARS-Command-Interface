...
- Version 30.3 opt-in immediate voice execution with Voice Authority enforcement, vocal authorization for protected direct actions, a contained masthead indicator, and accurate wake-word processing status
- Version 30.3 Module Platform with stable Extension API v3, explicit capability grants, RSA-SHA256 package signatures, Stable and Development repository channels, health monitoring, render isolation, one-step rollback, portable import/export, and Module Forge

## Current stage

Version 30.3 is the current Development milestone and Version 29 remains Stable. Its major feature is the Module Platform: a stable host-rendered API v3 contract, explicit capability grants, signed packages, Stable and Development channels, module health records, render isolation, rollback, portable packages, and the built-in Module Forge. API v2 and legacy checklist modules remain compatible. Voice commands still open Computer Core for review by default; operators may explicitly enable immediate execution, while Voice Authority and vocal authorization continue to guard protected actions. Version 30.2 Federation and the Version 30.1 Computer Core, Procedure, audit, undo, bundled transcription, and authorization safeguards remain intact. See `docs/MODULE-API-v3.md` for the public module contract.

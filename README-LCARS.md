...
Version 30.5 is the current Development milestone and Version 29 remains Stable. Universal LCARS Search covers applications, files and recent documents, settings, commands, stations, notifications, media, contacts, modules, Procedures, and activity history. Results can be opened, routed to a trusted station, or attached to a Procedure. The Data Fabric builds on Federation with selective synchronization, opt-in clipboard and small-file delivery, recent items, bounded record versions, explicit conflicts, and opt-in AES-256-GCM private storage. Voice operations now accept Green Alert and No Alert and distinguish one-command spoken confirmation from a compact sequence cue. Version 30.4 LCARS Session, Version 30.3 Module Platform, Version 30.2 Federation, and Version 30.1 Computer Core remain intact. See `docs/MODULE-API-v3.md` for the public module contract.

...
Version 30.4 is the current Development milestone and Version 29 remains Stable. Its major feature is an explicitly optional Linux LCARS login session built over existing Wayland/X11 desktop infrastructure. It registers only when requested in Settings and never silently becomes the default. Virtual desktops become decks; Workstations, multi-monitor routing, window tasking, placement rules, lock/logout/power, recovery, safe mode, and optional kiosk behavior form one environment with a normal-desktop escape route. Voice vocabulary now covers deterministic media transport, mute, locate/display/status/calendar/hailing commands, alerts, lock/logout, and protected close-only Self Destruct; failed commands no longer also receive an affirmative cue. Version 30.3 Module Platform, Version 30.2 Federation, and Version 30.1 Computer Core remain intact. See `docs/MODULE-API-v3.md` for the public module contract.

...
- Version 30.9 Commissioning and Trust Center with combined readiness checks, offline command discovery, privacy review, bounded retention, resume recovery, and continuous sidebar numbering
- Version 30.10 authoritative-session foundation with explicit shell ownership, real Meta+Shift+Escape recovery, clean-exit versus crash handling, continuous placement rules, and Task Rail routing to numbered decks
- Version 30.11 native System Control Matrix for Wi-Fi, Bluetooth, audio, displays, software, processes, storage, modules, and media without routine host control-panel handoffs
- Version 30 Stable viewport-fit audit with single-label Systems tabs, deliberate LCARS end caps, unclipped pages and pop-outs, and responsive layouts across desktop and handheld displays
...
- Version 31.4 Connectivity and Hardware with Portal-approved VPN/hotspot/saved-network changes, firewall state, connection diagnostics, peripheral inventory, and LCARS removable-device policy
- Version 31.5 Daily Utilities with private clipboard history, Portal-approved capture, notification service routing, print-queue management, and repaired Voice Control permissions

## Current stage

Version 31.5 Development is active on the `31-development` branch. It adds a compact Daily Utilities page for opt-in clipboard history, screenshots, screen recording, approved application notifications, and print-queue management; it also repairs trusted microphone permission handling for Voice Control. Existing Calendar, Communications, Search, Data Fabric, Files, and Operations surfaces are reused rather than duplicated. Version 30 remains the current Stable release. Read `docs/PROJECT-MEMORY.md` before planning a milestone; the deduplicated sequence is preserved in `docs/VERSION-31-ROADMAP.md`.
...
Version 29 introduced the Stable Mobile Command Environment and the persistent Android signing line retained by Version 30. It assembled the Version 29.1 Home foundation, Version 29.2 customization milestone, and the Connected Station Dock, multi-station, notification, credential-migration, battery, performance, calendar, Favorites, and mobile-update work. Only hashed revocable tokens are stored by the desktop core; Android station tokens are encrypted with Android Keystore AES-GCM. Viewer, Operator, Command, granular permission, and explicit sensitive-action approval checks remain enforced, and PADD devices never receive Terminal, file, process, or computer power access.

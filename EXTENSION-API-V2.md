...
## Version 26 Module Repository

Version 26 includes the official, download-only module repository backed by the project's dedicated `Modules` branch, operator-configured public GitHub repositories, and complete community-source backup and restore. Each community repository exposes a root `catalog.json`; LCARS labels it as community content and lets the operator add, disable, refresh, diagnose, or remove the source.
